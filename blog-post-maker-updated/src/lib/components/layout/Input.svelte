...
<div class="flex justify-center text-center">
	<div class="space-y-8 pt-14">
		<div>
			<p class="mb-2.5 ml-2.5 text-left text-gray-900 md:ml-0 md:pl-3">
			...
			<input
				...
				id="i-url"
				name="i-url"
				placeholder="https://xxxxxxxxxxxxxxxxxxx.supabase.co"
				class="md:w-120 w-80 rounded-xl border border-gray-300 py-2.5 pl-2 pr-8 text-sm outline-none duration-300 focus:border-gray-900"
			/>
		</div>
		<div>
			<p class="mb-2.5 ml-2.5 text-left text-gray-900 md:ml-0 md:pl-3">
			...
			{#if !showKey}
				<input
					...
					id="i-api"
					name="i-api"
					placeholder="xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx.xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
					class="md:w-120 w-80 rounded-xl border border-gray-300 py-2.5 pl-2 pr-8 text-sm outline-none duration-300 focus:border-gray-900"
				/>
			{:else}
				<input
			...
			{/if}
			<button
				class="absolute -translate-x-9 translate-y-2.5 pl-1.5 text-2xl text-gray-400 duration-300 hover:text-gray-500"
				on:click|preventDefault={() => (showKey = !showKey)}
			>
				{#if !showKey}
					<svg
